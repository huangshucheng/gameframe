...
function login_logic_server(s, req)
	...
	if p then -- 玩家对象已经存在了，更新一下session就可以了; 
		...
		print('user exist >> user size: '..  online_player_num)
		return
	end

	print('login_logic_server>>000 user size: '..  online_player_num)
	p = player:new()
	p:init(uid, s, function(status)
		if status == Respones.OK then
			logic_server_players[uid] = p
			online_player_num = online_player_num + 1
			print('login_logic_server>>111 user size: '..  online_player_num)
		end
		send_status(s, stype, Cmd.eLoginLogicRes, uid, status)
	end)
	print('login_logic_server>>222 user size: '..  online_player_num)
end

-- 玩家离开了
function on_player_disconnect(s, req)
	print('on_player_disconnect>>>')
	local uid = req[3]
