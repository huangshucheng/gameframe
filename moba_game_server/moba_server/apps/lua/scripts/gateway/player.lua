...
local function player:new(instant) 
	if not instant then 
		instant = {} --类的实例
	end
...
local function player:init(uid, s, ret_handler)
	self.session = s
	self.uid = uid
...
local function player:get_user_arrived()
	local body = {
...
local function player:set_session(s)
	self.session = s
end

local function player:send_cmd(stype, ctype, body)
